...
import (
	...
	valuation "customer/api/valuation"
...
func NewCustomerBiz(d *consul.Registry) (*CustomerBiz, func(), error) {
	...
	valuationClient := valuation.NewValuationClient(valConn)
	verifyCodeClient := verifyCode.NewVerifyCodeClient(verifyCodeConn)
	return &CustomerBiz{valuationClient, verifyCodeClient}, cleanup, nil
}

func (cb *CustomerBiz) GetEstimatePrice(ctx context.Context, origin, destination string) (int64, error) {
	reply, err := cb.valuation.GetEstimatePrice(ctx, &valuation.GetEstimatePriceRequest{
		Origin: origin,
